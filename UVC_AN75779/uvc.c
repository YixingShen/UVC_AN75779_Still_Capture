...
static uint8_t Still_Flag = 0;             /* Indicates whether a Still Trigger is received from the Host*/
static uint8_t Still_Frame_Strated = 0;   /* Indicates whether the current frame is a Still Frame*/
uint8_t Still_Image_Trigger_Control= 0;	  /* Indicates whether Still Trigger Control is received or not */
uint8_t Button_Press_Event = 0;			  /* Indicates whether a Vendor Command 0x88 representing Button press is received or not */
CyU3PDmaChannel Status_Interrupt_Channel;
static uint8_t Status_Packet[16];
...
/* UVC Still Probe Control for USB 3.0 Connection */
uint8_t glStillProbeCtrl30[CY_FX_MAX_STILL_PROBE_SETTING] ={
	0x01,						/*bFormatIndex: Use 1st Still Format Index*/
	0x01,						/*bFrameIndex: Use 1st Still Frame Index*/
	0x00,						/*bCompressionIndex: No Compression supported*/
	0x00,0x20,0x1C,0x00,		/*dwMaxVideoFrameSize: 1280*720*2 bytes*/
	0x00,0x40,0x00,0x00			/*dwMaxPayloadTransferSize:16KB DMA buffer size*/
...
static void Send_Status_to_Host(){

	CyU3PDmaBuffer_t Status_Buffer;
	CyU3PReturnStatus_t apiRetStatus = CY_U3P_SUCCESS;

	Status_Packet[0] = 0x02;  /* bStatusType = VideoStreaming interface */
	Status_Packet[1] = 0x01;  /* bOriginator = ID of VideoStreaming interface */
	Status_Packet[2] = 0x00;  /* bEvent      = Button press */
	Status_Packet[3] = 0x01;  /* bValue = Button press state */

	Status_Buffer.buffer = Status_Packet;
	Status_Buffer.count  = 4;
	Status_Buffer.size   = 16;
	Status_Buffer.status = 0;

		apiRetStatus = CyU3PDmaChannelSetupSendBuffer(&Status_Interrupt_Channel, &Status_Buffer);
		if (apiRetStatus != CY_U3P_SUCCESS)
			CyU3PDebugPrint(2, "SetupSendBuffer failed with 0x%x\n", apiRetStatus);

		apiRetStatus = CyU3PDmaChannelWaitForCompletion(&Status_Interrupt_Channel, CYU3P_WAIT_FOREVER);
		if (apiRetStatus != CY_U3P_SUCCESS)
    ...
    if ((bmReqType & CY_U3P_USB_TYPE_MASK) == CY_U3P_USB_VENDOR_RQT)
           {
           	...
           	{
           		case 0x88:
           			CyU3PDebugPrint(2, "Request still capture\n");
...
CyFxUVCApplnInit (void)
{
    ...
    /* Create a DMA Channel from CPU to Status Interrupt Endpoint*/
        ...
        dmaConfig.cb = 0;
           ...
           apiRetStatus = CyU3PDmaChannelCreate(&Status_Interrupt_Channel, CY_U3P_DMA_TYPE_MANUAL_OUT, &dmaConfig);
           if(apiRetStatus != CY_U3P_SUCCESS)
           {
        	...
        	case CY_FX_UVC_STILL_COMMIT_CONTROL:
        		switch(bRequest)
        		{
        			...
        			case CY_FX_USB_UVC_GET_DEF_REQ:
        				...
        				{
        					CyU3PUsbSendEP0Data (CY_FX_MAX_STILL_PROBE_SETTING, (uint8_t *)glStillProbeCtrl30);
        				}
        				break;
        			case CY_FX_USB_UVC_SET_CUR_REQ:
        				...
        				if(apiRetStatus == CY_U3P_SUCCESS){
        					if (usbSpeed == CY_U3P_SUPER_SPEED)
        					{
        						glStillProbeCtrl30[0] = glStillCommitCtrl[0];
        						glStillProbeCtrl30[1] = glStillCommitCtrl[1];
        					}
        	...
        	case CY_FX_UVC_STILL_TRIGGER_CONTROL:
        		switch(bRequest)
        		{
        			case CY_FX_USB_UVC_GET_INFO_REQ:
        	            ...
        	            break;
        			case CY_FX_USB_UVC_GET_CUR_REQ:
        				CyU3PUsbSendEP0Data (1, (uint8_t *)&Still_Image_Trigger_Control);
        				break;
        			case CY_FX_USB_UVC_SET_CUR_REQ:
        				apiRetStatus = CyU3PUsbGetEP0Data (CY_FX_MAX_STILL_PROBE_SETTING_ALIGNED,
        						&Still_Image_Trigger_Control, &readCount);
        				if (apiRetStatus == CY_U3P_SUCCESS){
        					if(Still_Image_Trigger_Control == 1)
        						Still_Flag = 1;
        				}
        				break;
        			default:
